...
const RUNTIME_DIR = path.join(ROOT, "phase04_runtime");
const COMPARE_DIR = path.join(ROOT, "phase04_compare");
...
async function executeCaseRerun({
  ...
  try {
    ...
    return {
      ...
      runtimeSource: {
        method: "phase04_broad_service_runner",
        generatedAt,
...
async function main() {
  ...
  let proceedToPostRerunQualityDiagnosis = false;
  ...
  const runtimeManifest = {
    ...
    runtimeGenerationMethodSummary:
      "Phase 4 broad service-runner execution using real local env with .env.local override, real DB, and real storage/OCR path via createCaseForUser + upsertPatientInput + uploadDocumentFile + createOcrJob; frozen earlier artifacts preserved; Case7 excluded from this rerun track.",
  ...
  proceedToPostRerunQualityDiagnosis =
    timeoutFixEffectivenessSupportedAtBroadScale &&
    compareStatuses.some((item) => item.comparisonStatus === "broad_partial_match" || item.comparisonStatus === "broad_match");
  ...
  const compareManifest = {
    ...
    highestRiskRemainingCases: failedRuntimeCases.map((item) => item.caseName),
    comparisonQuality:
      successfulRuntimeCases.length === attemptedCases.length && attemptedCases.length > 0
        ? "semantic-grade"
        : "execution-limited",
    timeoutFixEffectivenessSupportedAtBroadScale,
    proceedToPostRerunQualityDiagnosis,
  ...
  await writeJson(path.join(ROOT, "phase04_runtime_manifest.json"), runtimeManifest);
  await writeJson(path.join(ROOT, "phase04_compare_manifest.json"), compareManifest);
}
