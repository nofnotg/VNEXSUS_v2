...
type CompareArtifact = {
  ...
  comparisonStatus:
    | "tailrisk_resolved"
    | "tailrisk_improved_partial_match"
    | "tailrisk_unchanged_partial_match"
    | "tailrisk_regressed"
    | "tailrisk_failed"
    | "needs_human_review";
  latestLockedBaselinePhase: "phase10";
  timeoutLikeRuntimeFailureRecurred: boolean;
  dominantResidualGap: string;
  dominantBlockerImproved: boolean;
  previousBlockerRemainedUnchanged: boolean;
  newBlockerAppeared: string | null;
  lowEndRiskState: "low_end_risk_cleared" | "low_end_risk_possible" | "low_end_risk_present";
...
const RUNTIME_DIR = path.join(ROOT, "phase12_6_runtime");
const COMPARE_DIR = path.join(ROOT, "phase12_6_compare");
const MANIFEST_PATH = path.join(ROOT, "phase12_6_validation_manifest.json");
const VALIDATION_ORDER = ["Case3", "Case10", "Case36"] as const;
...
function getLatestLockedBaselinePhase(): "phase10" {
  return "phase10";
}

function getLatestRuntimePath(caseName: string, _phase: "phase10") {
  return path.join(ROOT, "phase10_runtime", `${caseName}_runtime_refresh.json`);
}

function getLatestComparePath(caseName: string, _phase: "phase10") {
  return path.join(ROOT, "phase10_compare", `${caseName}_compare_refresh.json`);
}
...
function getLatestMissingDates(compare: Phase04CompareArtifact) {
  return (
    compare.dateCompletenessSummary?.refreshMissingDates ??
...
function getLatestExtraDates(compare: Phase04CompareArtifact) {
  return (
    compare.dateCompletenessSummary?.refreshExtraDates ??
...
function buildCompareArtifact(
  ...
  const previousMissingDates = getLatestMissingDates(previousCompare);
  const previousExtraDates = getLatestExtraDates(previousCompare);
  const previousDominantMetric = caseName === "Case3" ? previousMissingDates.length : previousExtraDates.length;
  const currentDominantMetric = caseName === "Case3" ? missingDates.length : extraDates.length;

  let comparisonStatus: CompareArtifact["comparisonStatus"] = "tailrisk_unchanged_partial_match";
  if (!runtime.pipelineStageStatus.ocrCompleted || !runtime.pipelineStageStatus.eventBundlesAvailable) {
    comparisonStatus = "tailrisk_failed";
  } else if (
    currentDominantMetric <= Math.max(1, Math.floor(previousDominantMetric / 2)) &&
    missingDates.length + extraDates.length <= Math.max(4, previousMissingDates.length + previousExtraDates.length - 4)
  ) {
    comparisonStatus = "tailrisk_resolved";
  } else if (runtime.relativeToLatestLockedBaseline === "better") {
    comparisonStatus = "tailrisk_improved_partial_match";
  } else if (runtime.relativeToLatestLockedBaseline === "worse") {
    comparisonStatus = "tailrisk_regressed";
  }

  const lowEndRiskState: CompareArtifact["lowEndRiskState"] =
    comparisonStatus === "tailrisk_resolved"
      ? "low_end_risk_cleared"
      : comparisonStatus === "tailrisk_improved_partial_match"
        ? "low_end_risk_possible"
        : comparisonStatus === "tailrisk_unchanged_partial_match" && currentDominantMetric <= Math.max(3, previousDominantMetric - 1)
          ? "low_end_risk_possible"
  ...
  return {
    ...
    lowEndRiskState,
    dateCompletenessSummary: {
      previousMissingDates,
      currentMissingDates: missingDates,
      previousExtraDates,
      currentExtraDates: extraDates
    },
    institutionCompletenessSummary: {
      previousMissingInstitutions: getLatestMissingInstitutions(previousCompare),
      currentMissingInstitutions: missingInstitutions,
      previousExtraInstitutions: getLatestExtraInstitutions(previousCompare),
      currentExtraInstitutions: extraInstitutions
    },
    bundleSummary: {
      previousBundleCount: previousRuntime.normalizedEventCoverageSummary.eventBundleCount,
      currentBundleCount: runtime.normalizedEventCoverageSummary.eventBundleCount
    },
    ...
    evidenceBackedNotes: [
      ...
      `previous_dominant_metric=${previousDominantMetric}`,
      `current_dominant_metric=${currentDominantMetric}`,
      `low_end_risk_state=${lowEndRiskState}`
    ],
...
async function writeRuntimeArtifact(caseName: string, runtime: RuntimeArtifact) {
  await writeJson(path.join(RUNTIME_DIR, `${caseName}_runtime_tailrisk.json`), runtime);
}
...
async function executeCaseRerun({
  ...
  try {
    const created = await createCaseForUser(investigator.id, {
      title: `Phase 12.6 ${pilotCase.caseName}`,
    ...
    return {
      ...
      track,
      runtimeSource: {
        method: "phase12_6_tailrisk_service_runner",
        generatedAt,
...
async function main() {
  ...
  const dominantResidualGapByCase = new Map<string, string>([
    ["Case3", "minor_date_missing_residual"],
    ["Case10", "minor_date_noise_residual"],
    ["Case36", "minor_date_noise_residual"]
  ]);
  ...
  const perCaseLowEndRiskState: Array<{
    caseName: string;
    lowEndRiskState: CompareArtifact["lowEndRiskState"];
  }> = [];
  const perCaseTailRiskState: Array<{
    caseName: string;
    tailRiskState: "resolved" | "reduced" | "unchanged" | "worsened";
  }> = [];
  let phase12_6SuccessfullyReducedTailRiskSet = false;
  let anyProductBlockingTailRiskCase = false;
  ...
  for (const caseName of VALIDATION_ORDER) {
    ...
    const latestLockedBaselinePhase = getLatestLockedBaselinePhase();
    ...
    const runtime = await executeCaseRerun({
      ...
      previousCompare,
      previousRuntime,
      dominantResidualGap: dominantResidualGapByCase.get(caseName) ?? "minor_date_noise_residual"
    });

    const runtimePath = path.join(RUNTIME_DIR, `${caseName}_runtime_tailrisk.json`);
    ...
    const comparePath = path.join(COMPARE_DIR, `${caseName}_compare_tailrisk.json`);
    ...
    perCaseLowEndRiskState.push({ caseName, lowEndRiskState: compareArtifact.lowEndRiskState });
    const tailRiskState =
      compareArtifact.comparisonStatus === "tailrisk_resolved"
        ? "resolved"
        : compareArtifact.comparisonStatus === "tailrisk_improved_partial_match"
          ? "reduced"
          : compareArtifact.comparisonStatus === "tailrisk_regressed"
            ? "worsened"
            : "unchanged";
    perCaseTailRiskState.push({ caseName, tailRiskState });

    if (compareArtifact.comparisonStatus === "tailrisk_failed" || compareArtifact.comparisonStatus === "tailrisk_regressed") {
      failedValidationCases.push(caseName);
    } else {
      successfulValidationCases.push(caseName);
    }
    phase12_6SuccessfullyReducedTailRiskSet ||= tailRiskState === "resolved" || tailRiskState === "reduced";
    anyRegression ||= tailRiskState === "worsened";
    anyProductBlockingTailRiskCase ||= compareArtifact.lowEndRiskState === "low_end_risk_present";
  }

  const readyForRenewedStabilizationReview =
    !timeoutLikeRuntimeFailureRecurred &&
    !anyRegression &&
    !anyProductBlockingTailRiskCase;
  ...
  const validationManifest = {
    ...
    perCaseTailRiskState,
    timeoutLikeRuntimeFailureRecurred,
    phase12SuccessfullyReducedTailRiskSet: phase12_6SuccessfullyReducedTailRiskSet,
    anyProductBlockingTailRiskCase,
    readyForRenewedStabilizationReview,
    anyRegression,
