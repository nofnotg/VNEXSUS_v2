...
type RuntimeArtifact = {
  ...
  relativeToLatestLockedBaseline: "better" | "same" | "worse";
  notes: string[];
};
...
type CompareArtifact = {
  ...
  runtimeFile: string;
  comparisonStatus:
    | "refresh_improved_partial_match"
    | "refresh_unchanged_partial_match"
    | "refresh_match"
    | "refresh_regressed"
    | "refresh_failed"
    | "needs_human_review";
  latestLockedBaselinePhase: "phase06_5" | "phase07" | "phase09_5" | "phase09_7";
  ...
  dateCompletenessSummary: {
    latestMissingDates: string[];
    refreshMissingDates: string[];
    latestExtraDates: string[];
    refreshExtraDates: string[];
  };
  institutionCompletenessSummary: {
    latestMissingInstitutions: string[];
    refreshMissingInstitutions: string[];
    latestExtraInstitutions: string[];
    refreshExtraInstitutions: string[];
  };
  bundleSummary: {
...
type Phase04CompareArtifact = {
  ...
  institutionCompletenessSummary?: {
    currentMissingInstitutions?: string[];
    currentExtraInstitutions?: string[];
...
const RUNTIME_DIR = path.join(ROOT, "phase10_runtime");
const COMPARE_DIR = path.join(ROOT, "phase10_compare");
const MANIFEST_PATH = path.join(ROOT, "phase10_validation_manifest.json");
const VALIDATION_ORDER = ["Case16", "Case3", "Case17", "Case2", "Case20", "Case34", "Case12", "Case10", "Case36"] as const;
...
function getLatestLockedBaselinePhase(caseName: string): "phase06_5" | "phase07" | "phase09_5" | "phase09_7" {
  if (caseName === "Case2" || caseName === "Case16" || caseName === "Case34") {
    return "phase09_7";
  }
  if (caseName === "Case12") {
    return "phase07";
...
function getLatestRuntimePath(caseName: string, phase: "phase06_5" | "phase07" | "phase09_5" | "phase09_7") {
  switch (phase) {
    case "phase06_5":
      return path.join(ROOT, "phase06_5_runtime", `${caseName}_runtime_postfix.json`);
    case "phase07":
      return path.join(ROOT, "phase07_runtime", `${caseName}_runtime_broader_postfix.json`);
    case "phase09_5":
      return path.join(ROOT, "phase09_5_runtime", `${caseName}_runtime_postfix.json`);
    case "phase09_7":
      return path.join(ROOT, "phase09_7_runtime", `${caseName}_runtime_track2fix.json`);
  }
}

function getLatestComparePath(caseName: string, phase: "phase06_5" | "phase07" | "phase09_5" | "phase09_7") {
  switch (phase) {
    case "phase06_5":
      return path.join(ROOT, "phase06_5_compare", `${caseName}_compare_postfix.json`);
    case "phase07":
      return path.join(ROOT, "phase07_compare", `${caseName}_compare_broader_postfix.json`);
    case "phase09_5":
      return path.join(ROOT, "phase09_5_compare", `${caseName}_compare_postfix.json`);
    case "phase09_7":
      return path.join(ROOT, "phase09_7_compare", `${caseName}_compare_track2fix.json`);
  }
}
...
function getLatestMissingDates(compare: Phase04CompareArtifact) {
  return (
    compare.dateCompletenessSummary?.currentMissingDates ??
...
function getLatestExtraDates(compare: Phase04CompareArtifact) {
  return (
    compare.dateCompletenessSummary?.currentExtraDates ??
...
function getLatestMissingInstitutions(compare: Phase04CompareArtifact) {
  return (
    compare.institutionCompletenessSummary?.currentMissingInstitutions ??
...
function getLatestExtraInstitutions(compare: Phase04CompareArtifact) {
  return (
    compare.institutionCompletenessSummary?.currentExtraInstitutions ??
...
function buildCompareArtifact(
  ...
  let comparisonStatus: CompareArtifact["comparisonStatus"] = "refresh_unchanged_partial_match";
  if (!runtime.pipelineStageStatus.ocrCompleted || !runtime.pipelineStageStatus.eventBundlesAvailable) {
    comparisonStatus = "refresh_failed";
  } else if (missingDates.length === 0 && extraDates.length === 0 && missingInstitutions.length === 0 && extraInstitutions.length === 0) {
    comparisonStatus = "refresh_match";
  } else if (runtime.relativeToLatestLockedBaseline === "better") {
    comparisonStatus = "refresh_improved_partial_match";
  } else if (runtime.relativeToLatestLockedBaseline === "worse") {
    comparisonStatus = "refresh_regressed";
  }
  ...
  return {
    ...
    dateCompletenessSummary: {
      latestMissingDates: getLatestMissingDates(previousCompare),
      refreshMissingDates: missingDates,
      latestExtraDates: getLatestExtraDates(previousCompare),
      refreshExtraDates: extraDates
    },
    institutionCompletenessSummary: {
      latestMissingInstitutions: getLatestMissingInstitutions(previousCompare),
      refreshMissingInstitutions: missingInstitutions,
      latestExtraInstitutions: getLatestExtraInstitutions(previousCompare),
      refreshExtraInstitutions: extraInstitutions
    },
    bundleSummary: {
      latestBundleCount: previousRuntime.normalizedEventCoverageSummary.eventBundleCount,
      refreshBundleCount: runtime.normalizedEventCoverageSummary.eventBundleCount
    },
    remainingSemanticQualityGaps,
    evidenceBackedNotes: [
      `relative_to_latest_locked_baseline=${runtime.relativeToLatestLockedBaseline}`,
      `dominant_blocker_improved=${runtime.dominantBlockerImproved}`,
      `timeout_like_recurred=${runtime.timeoutLikeRuntimeFailureRecurred}`
    ],
    relativeToLatestLockedBaseline: runtime.relativeToLatestLockedBaseline,
...
async function writeRuntimeArtifact(caseName: string, runtime: RuntimeArtifact) {
  await writeJson(path.join(RUNTIME_DIR, `${caseName}_runtime_refresh.json`), runtime);
}
...
async function executeCaseRerun({
  ...
  try {
    const created = await createCaseForUser(investigator.id, {
      title: `Phase 10 ${pilotCase.caseName}`,
    ...
    const missingDates = goldenDates.filter((value) => !runtimeDates.includes(value));
    const extraDates = runtimeDates.filter((value) => !goldenDates.includes(value));
    const latestExtraDates = getLatestExtraDates(previousCompare).length;
    const latestMissingDates = getLatestMissingDates(previousCompare).length;
    ...
    const previousInstitutionGap =
      getLatestMissingInstitutions(previousCompare).length + getLatestExtraInstitutions(previousCompare).length;
    ...
    let relativeToLatestLockedBaseline: "better" | "same" | "worse" = "same";

    if (track === "date") {
      const previousTotalGap = latestExtraDates + latestMissingDates;
      const currentTotalGap = extraDates.length + missingDates.length;
      dominantBlockerImproved = currentTotalGap < previousTotalGap && missingDates.length <= latestMissingDates + 1;
      previousBlockerRemainedUnchanged = extraDates.length === latestExtraDates && missingDates.length === latestMissingDates;
      relativeToLatestLockedBaseline =
    ...
    return {
      ...
      track,
      runtimeSource: {
        method: "phase10_refresh_service_runner",
        generatedAt,
    ...
    return {
      ...
      relativeToLatestLockedBaseline: "worse",
      notes: [`budgetEstimatedCostUsd=${pilotCase.estimatedCost}`]
    };
  }
}
...
async function main() {
  ...
  const perTrackBroaderScaleEffectivenessSummary = {
  ...
  const perCaseImprovementVsLatestLockedBaseline: Array<{
    caseName: string;
    status: CompareArtifact["comparisonStatus"];
    relativeToLatestLockedBaseline: RuntimeArtifact["relativeToLatestLockedBaseline"];
  }> = [];
  const highRiskRemainingCases: string[] = [];
  ...
  for (const caseName of VALIDATION_ORDER) {
    ...
    const latestLockedBaselinePhase = getLatestLockedBaselinePhase(caseName);
    const previousRuntime = await readJson<Phase04RuntimeArtifact>(
      getLatestRuntimePath(caseName, latestLockedBaselinePhase)
    );
    const previousCompare = await readJson<Phase04CompareArtifact>(
      getLatestComparePath(caseName, latestLockedBaselinePhase)
    );
    ...
    const runtimePath = path.join(RUNTIME_DIR, `${caseName}_runtime_refresh.json`);
    ...
    const comparePath = path.join(COMPARE_DIR, `${caseName}_compare_refresh.json`);
    ...
    if (compareArtifact.comparisonStatus === "refresh_failed" || compareArtifact.comparisonStatus === "refresh_regressed") {
      failedValidationCases.push(caseName);
    } else {
      successfulValidationCases.push(caseName);
    }
    perCaseImprovementVsLatestLockedBaseline.push({
      caseName,
      status: compareArtifact.comparisonStatus,
      relativeToLatestLockedBaseline: runtime.relativeToLatestLockedBaseline
    });
    ...
    const judgment =
      compareArtifact.comparisonStatus === "refresh_match" || compareArtifact.comparisonStatus === "refresh_improved_partial_match"
        ? "improved"
        : compareArtifact.comparisonStatus === "refresh_regressed"
          ? "regressed"
          : "unchanged";

    if (runtime.track === "date") {
      if (judgment === "regressed") perTrackBroaderScaleEffectivenessSummary.dateNormalization = "regressed";
      else if (judgment === "improved" && perTrackBroaderScaleEffectivenessSummary.dateNormalization !== "regressed") {
        perTrackBroaderScaleEffectivenessSummary.dateNormalization = "improved";
      }
    } else if (runtime.track === "institution") {
      if (judgment === "regressed") perTrackBroaderScaleEffectivenessSummary.institutionNormalization = "regressed";
      else if (judgment === "improved" && perTrackBroaderScaleEffectivenessSummary.institutionNormalization !== "regressed") {
        perTrackBroaderScaleEffectivenessSummary.institutionNormalization = "improved";
      }
    } else {
      perTrackBroaderScaleEffectivenessSummary.bundleGrouping = judgment;
    }

    anyRegression ||= judgment === "regressed";
    if (compareArtifact.comparisonStatus === "refresh_regressed" || compareArtifact.comparisonStatus === "refresh_failed") {
      highRiskRemainingCases.push(caseName);
    }
  }

  const recoverableSetReadyForFinalConvergenceReview =
    !timeoutLikeRuntimeFailureRecurred &&
    perTrackBroaderScaleEffectivenessSummary.dateNormalization !== "regressed" &&
    perTrackBroaderScaleEffectivenessSummary.institutionNormalization !== "regressed" &&
    perTrackBroaderScaleEffectivenessSummary.bundleGrouping !== "regressed" &&
    !anyRegression;
  ...
  const validationManifest = {
    ...
    perCaseStatus,
    perCaseImprovementVsLatestLockedBaseline,
    perTrackBroaderScaleEffectivenessSummary,
    timeoutLikeRuntimeFailureRecurred,
    highRiskRemainingCases,
    recoverableSetReadyForFinalConvergenceReview,
    anyRegression,
  ...
  await writeJson(MANIFEST_PATH, validationManifest);
}
