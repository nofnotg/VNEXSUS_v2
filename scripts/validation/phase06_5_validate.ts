...
type CompareArtifact = {
  ...
  runtimeFile: string;
  comparisonStatus:
    | "postfix_improved_partial_match"
    | "postfix_unchanged_partial_match"
    | "postfix_regressed"
    | "postfix_failed"
    | "needs_human_review";
...
const RUNTIME_DIR = path.join(ROOT, "phase06_5_runtime");
const COMPARE_DIR = path.join(ROOT, "phase06_5_compare");
const VALIDATION_ORDER = ["Case17", "Case10", "Case2", "Case34", "Case20"] as const;
...
function determineTrack(caseName: string): "date" | "institution" | "bundle" {
  ...
  if (caseName === "Case2" || caseName === "Case34") {
    return "institution";
  }
  return "date";
}
...
function buildCompareArtifact(
  ...
  let comparisonStatus: CompareArtifact["comparisonStatus"] = "postfix_unchanged_partial_match";
  if (!runtime.pipelineStageStatus.ocrCompleted || !runtime.pipelineStageStatus.eventBundlesAvailable) {
    comparisonStatus = "postfix_failed";
  } else if (runtime.relativeToPhase4 === "better") {
    comparisonStatus = "postfix_improved_partial_match";
  } else if (runtime.relativeToPhase4 === "worse") {
    comparisonStatus = "postfix_regressed";
  }
...
async function executeCaseRerun({
  ...
  try {
    ...
    return {
      ...
      track,
      runtimeSource: {
        method: "phase06_5_service_runner",
        generatedAt,
...
async function main() {
  ...
  for (const caseName of VALIDATION_ORDER) {
    ...
    if (compareArtifact.comparisonStatus === "postfix_failed" || compareArtifact.comparisonStatus === "postfix_regressed") {
    ...
    const judgment =
      compareArtifact.comparisonStatus === "postfix_improved_partial_match"
        ? "improved"
        : compareArtifact.comparisonStatus === "postfix_regressed"
          ? "regressed"
          : "unchanged";
  ...
  await writeJson(path.join(ROOT, "phase06_5_validation_manifest.json"), validationManifest);
}
