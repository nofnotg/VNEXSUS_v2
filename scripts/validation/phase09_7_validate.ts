...
type CompareArtifact = {
  ...
  comparisonStatus:
    | "track2_resolved"
    | "track2_improved_partial_match"
    | "track2_unchanged_partial_match"
    | "track2_regressed"
    | "track2_failed"
    | "needs_human_review";
  latestLockedBaselinePhase: "phase09_5";
  institutionAliasDriftImproved: boolean;
  canonicalHospitalIdentityMoreStable: boolean;
  downstreamCarryForwardImproved: boolean;
  newFalseMergesAppeared: boolean;
  timeoutLikeRuntimeFailureRecurred: boolean;
  resultVsLatestLockedBaseline: "better" | "same" | "worse";
  ...
  bundleInstitutionSummary: {
    previousCombinedInstitutionCount: number;
    currentCombinedInstitutionCount: number;
...
const RUNTIME_DIR = path.join(ROOT, "phase09_7_runtime");
const COMPARE_DIR = path.join(ROOT, "phase09_7_compare");
const MANIFEST_PATH = path.join(ROOT, "phase09_7_validation_manifest.json");
const VALIDATION_ORDER = ["Case2", "Case16", "Case34"] as const;
...
function determineTrack(): "institution" {
  return "institution";
}
...
function buildCompareArtifact(
  ...
  const previousMissingInstitutions = getPreviousMissingInstitutions(previousCompare);
  const previousExtraInstitutions = getPreviousExtraInstitutions(previousCompare);
  const currentMissingInstitutions = normalizedGoldenInstitutions.filter((value) => !normalizedRuntimeInstitutions.includes(value));
  const currentExtraInstitutions = normalizedRuntimeInstitutions.filter((value) => !normalizedGoldenInstitutions.includes(value));
  const previousGap = previousMissingInstitutions.length + previousExtraInstitutions.length;
  const currentGap = currentMissingInstitutions.length + currentExtraInstitutions.length;
  const previousCombinedInstitutionCount = previousRuntime.normalizedInstitutionSummary.combinedInstitutions.length;
  const currentCombinedInstitutionCount = runtime.normalizedInstitutionSummary.combinedInstitutions.length;

  const institutionAliasDriftImproved = currentExtraInstitutions.length < previousExtraInstitutions.length;
  const canonicalHospitalIdentityMoreStable =
    currentCombinedInstitutionCount <= previousCombinedInstitutionCount &&
    currentExtraInstitutions.length <= previousExtraInstitutions.length;
  const downstreamCarryForwardImproved = currentMissingInstitutions.length < previousMissingInstitutions.length;
  const newFalseMergesAppeared =
    currentMissingInstitutions.length > previousMissingInstitutions.length &&
    currentCombinedInstitutionCount < previousCombinedInstitutionCount;

  let comparisonStatus: CompareArtifact["comparisonStatus"] = "track2_unchanged_partial_match";
  if (!runtime.pipelineStageStatus.ocrCompleted || !runtime.pipelineStageStatus.eventBundlesAvailable) {
    comparisonStatus = "track2_failed";
  } else if (currentGap === 0 && !newFalseMergesAppeared) {
    comparisonStatus = "track2_resolved";
  } else if (runtime.relativeToPreviousLockedBaseline === "better") {
    comparisonStatus = "track2_improved_partial_match";
  } else if (runtime.relativeToPreviousLockedBaseline === "worse") {
    comparisonStatus = "track2_regressed";
  }
  ...
  return {
    ...
    comparisonStatus,
    latestLockedBaselinePhase: "phase09_5",
    institutionAliasDriftImproved,
    canonicalHospitalIdentityMoreStable,
    downstreamCarryForwardImproved,
    newFalseMergesAppeared,
    timeoutLikeRuntimeFailureRecurred: runtime.timeoutLikeRuntimeFailureRecurred,
    resultVsLatestLockedBaseline: runtime.relativeToPreviousLockedBaseline,
    institutionCompletenessSummary: {
      previousMissingInstitutions,
      currentMissingInstitutions,
      previousExtraInstitutions,
      currentExtraInstitutions
    },
    bundleInstitutionSummary: {
      previousCombinedInstitutionCount,
      currentCombinedInstitutionCount,
    ...
    reviewNotes: [
      `result_vs_latest_locked_baseline=${runtime.relativeToPreviousLockedBaseline}`,
      `institution_alias_drift_improved=${institutionAliasDriftImproved}`,
      `canonical_identity_more_stable=${canonicalHospitalIdentityMoreStable}`,
      `carry_forward_improved=${downstreamCarryForwardImproved}`,
      `new_false_merges=${newFalseMergesAppeared}`,
...
async function writeRuntimeArtifact(caseName: string, runtime: RuntimeArtifact) {
  await writeJson(path.join(RUNTIME_DIR, `${caseName}_runtime_track2fix.json`), runtime);
}
...
async function executeCaseRerun({
  ...
  try {
    const created = await createCaseForUser(investigator.id, {
      title: `Phase 9.7 ${pilotCase.caseName}`,
    ...
    const relativeToPreviousLockedBaseline: "better" | "same" | "worse" =
      currentInstitutionGap < previousInstitutionGap ? "better" : currentInstitutionGap > previousInstitutionGap ? "worse" : "same";

    return {
      caseName: pilotCase.caseName,
      track,
      runtimeSource: {
        method: "phase09_7_track2_service_runner",
        generatedAt,
      ...
      dominantResidualGap: "minor_institution_alias_residual",
...
async function main() {
  ...
  let anyHighRiskInstitutionException = false;
  let case2RegressionState: "resolved" | "reduced" | "unchanged" | "worsened" = "unchanged";
  let case16StayedNonRegressed = true;
  let case34StayedNonRegressed = true;
  ...
  for (const caseName of VALIDATION_ORDER) {
    ...
    const previousRuntime = await readJson<Phase04RuntimeArtifact>(
      path.join(ROOT, "phase09_5_runtime", `${caseName}_runtime_postfix.json`)
    );
    const previousCompare = await readJson<Phase04CompareArtifact>(
      path.join(ROOT, "phase09_5_compare", `${caseName}_compare_postfix.json`)
    );
    ...
    const runtimePath = path.join(RUNTIME_DIR, `${caseName}_runtime_track2fix.json`);
    await writeRuntimeArtifact(caseName, runtime);
    generatedRuntimeFiles.push(runtimePath);
    timeoutLikeRuntimeFailureRecurred ||= runtime.timeoutLikeRuntimeFailureRecurred;

    const compareArtifact = buildCompareArtifact(caseName, golden, runtimePath, runtime, previousRuntime, previousCompare);
    const comparePath = path.join(COMPARE_DIR, `${caseName}_compare_track2fix.json`);
    ...
    if (compareArtifact.comparisonStatus === "track2_failed" || compareArtifact.comparisonStatus === "track2_regressed") {
    ...
    if (caseName === "Case2") {
      if (compareArtifact.comparisonStatus === "track2_resolved") {
        case2RegressionState = "resolved";
      } else if (compareArtifact.comparisonStatus === "track2_improved_partial_match") {
        case2RegressionState = "reduced";
      } else if (compareArtifact.comparisonStatus === "track2_regressed") {
        case2RegressionState = "worsened";
      } else {
        case2RegressionState = "unchanged";
      }
    }
    if (caseName === "Case16") case16StayedNonRegressed = !["track2_failed", "track2_regressed"].includes(compareArtifact.comparisonStatus);
    if (caseName === "Case34") case34StayedNonRegressed = !["track2_failed", "track2_regressed"].includes(compareArtifact.comparisonStatus);

    anyRegression ||= compareArtifact.comparisonStatus === "track2_regressed";
    anyHighRiskInstitutionException ||= compareArtifact.newFalseMergesAppeared || compareArtifact.comparisonStatus === "needs_human_review";
  }

  const track2SafeEnoughForBroaderRerun =
    !timeoutLikeRuntimeFailureRecurred &&
    (case2RegressionState === "resolved" || case2RegressionState === "reduced") &&
    case16StayedNonRegressed &&
    case34StayedNonRegressed &&
    !anyHighRiskInstitutionException;
  ...
  const validationManifest = {
    ...
    perCaseStatus,
    case2RegressionState,
    case16StayedNonRegressed,
    case34StayedNonRegressed,
    track2SafeEnoughForBroaderRerun,
    anyHighRiskInstitutionException,
    timeoutLikeRuntimeFailureRecurred,
    anyRegression,
