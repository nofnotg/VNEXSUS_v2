...
type RuntimeArtifact = {
  ...
  broadStatus: "broad_completed" | "broad_failed";
...
const RUNTIME_DIR = path.join(ROOT, "broader_rerun_runtime");
const COMPARE_DIR = path.join(ROOT, "broader_rerun_compare");
const MANIFEST_PATH = path.join(ROOT, "broader_rerun_manifest.json");
const SUMMARY_PATH = path.join(ROOT, "broader_rerun_summary.json");
const BROAD_ORDER = ["Case16", "Case3", "Case17", "Case2", "Case20", "Case34", "Case12", "Case10", "Case36"] as const;
const SMOKE_CASE = "Case16";
const INVESTIGATOR_EMAIL = "charmorzr@gmail.com";
const WATCH_CASE = "Case3";
const EXCLUDED_CASE = "Case7";
...
function hasDate(runtime: RuntimeArtifact, targetDate: string) {
  return [
...
function toLowEndRiskState(comparisonStatus: CompareArtifact["comparisonStatus"]) {
  if (comparisonStatus === "broad_match") {
    return "low_end_risk_cleared" as const;
  }

  if (comparisonStatus === "broad_partial_match" || comparisonStatus === "needs_human_review") {
    return "low_end_risk_possible" as const;
  }

  return "low_end_risk_present" as const;
}
...
async function writeRuntimeArtifact(caseName: string, runtime: RuntimeArtifact) {
  await writeJson(path.join(RUNTIME_DIR, `${caseName}_runtime_broad.json`), runtime);
}
...
async function executeCaseRerun({
  ...
  try {
    const created = await createCaseForUser(investigator.id, {
      title: `Broader Rerun ${pilotCase.caseName}`,
    ...
    return {
      ...
      runtimeSource: {
        method: "phase12_6_broader_rerun_service_runner",
        generatedAt,
      ...
      broadStatus: pipelineStageStatus.ocrCompleted ? "broad_completed" : "broad_failed",
...
async function main() {
  ...
  const successfulRuntimeCases: string[] = [];
  const failedRuntimeCases: Array<{ caseName: string; reason: string }> = [];
  const generatedRuntimeFiles: string[] = [];
  const generatedCompareFiles: string[] = [];
  const runtimeByCase = new Map<string, RuntimeArtifact>();
  const compareByCase = new Map<string, CompareArtifact>();
  let timeoutFixEffectivenessSupportedAtBroadScale = false;
  let priorSemanticTimeoutSymptomRecurred = false;

  for (const caseName of BROAD_ORDER) {
    ...
    generatedRuntimeFiles.push(path.join(RUNTIME_DIR, `${caseName}_runtime_broad.json`));
    runtimeByCase.set(caseName, runtime);
    priorSemanticTimeoutSymptomRecurred ||= runtime.priorSemanticTimeoutSymptomRecurred;

    if (attemptedCases.length <= 3) {
      ...
      if (sameCredentialIssue || runtime.priorSemanticTimeoutSymptomRecurred || !runtime.pipelineStageStatus.eventBundlesAvailable) {
        failedRuntimeCases.push({
          caseName,
          reason: sameCredentialIssue
            ? "same_credential_path_failure"
            : runtime.priorSemanticTimeoutSymptomRecurred
              ? "semantic_timeout_recurred_in_broad_frontier"
              : "frontier_case_bundle_persistence_incomplete"
        });
        const failedCompareArtifact = buildCompareArtifact(
          caseName,
          golden,
          path.join(RUNTIME_DIR, `${caseName}_runtime_broad.json`),
          runtime
        );
        await writeJson(path.join(COMPARE_DIR, `${caseName}_compare_broad.json`), failedCompareArtifact);
        generatedCompareFiles.push(path.join(COMPARE_DIR, `${caseName}_compare_broad.json`));
        compareByCase.set(caseName, failedCompareArtifact);
        break;
      }
    }
    ...
    await writeJson(path.join(COMPARE_DIR, `${caseName}_compare_broad.json`), compareArtifact);
    generatedCompareFiles.push(path.join(COMPARE_DIR, `${caseName}_compare_broad.json`));
    compareByCase.set(caseName, compareArtifact);
    ...
    if (
      ...
      failedRuntimeCases.push({
        ...
        reason:
          ...
          (!runtime.pipelineStageStatus.eventBundlesAvailable ? "bundle_persistence_incomplete" : "broad_failed")
      });
      continue;
    }

    successfulRuntimeCases.push(caseName);
  }

  timeoutFixEffectivenessSupportedAtBroadScale =
    !priorSemanticTimeoutSymptomRecurred && successfulRuntimeCases.length >= 3;

  const compareStatuses = BROAD_ORDER.filter((caseName) => compareByCase.has(caseName)).map((caseName) => {
    const compare = compareByCase.get(caseName)!;
    return { caseName, comparisonStatus: compare.comparisonStatus };
  });

  const perCaseLowEndRiskState = compareStatuses.map((item) => ({
    caseName: item.caseName,
    lowEndRiskState: toLowEndRiskState(item.comparisonStatus)
  }));
  ...
  const regressionCases = compareStatuses
    .filter((item) => item.comparisonStatus === "broad_failed" || item.comparisonStatus === "broad_mismatch")
    .map((item) => item.caseName);
  const anyRegression = regressionCases.length > 0;
  const anyLowEndRiskPresent = perCaseLowEndRiskState.some((item) => item.lowEndRiskState === "low_end_risk_present");

  const case3Runtime = runtimeByCase.get("Case3");
  const case10Runtime = runtimeByCase.get("Case10");
  const case36Runtime = runtimeByCase.get("Case36");
  const case3Compare = compareByCase.get("Case3");
  const case10Compare = compareByCase.get("Case10");
  const case36Compare = compareByCase.get("Case36");

  const case3Watch = {
    caseName: WATCH_CASE,
    remainsWatchItem: true,
    blockerEscalated:
      !case3Runtime ||
      !case3Compare ||
      case3Compare.comparisonStatus === "broad_failed" ||
      case3Compare.comparisonStatus === "broad_mismatch" ||
      !case3Runtime.pipelineStageStatus.eventBundlesAvailable,
    missingDateSensitivityNote:
      case3Compare && case3Runtime
        ? `missingDates=${case3Compare.dateCoverageComparison.missingDates.length};extraDates=${case3Compare.dateCoverageComparison.extraDates.length};bundleCount=${case3Runtime.normalizedEventCoverageSummary.eventBundleCount}`
        : "case3_runtime_or_compare_missing"
  };

  const case10Protection = {
    runtimeContains2024_10_25: case10Runtime ? hasDate(case10Runtime, "2024-10-25") : null,
    runtimeContains2024_10_30: case10Runtime ? hasDate(case10Runtime, "2024-10-30") : null,
    stableImproved:
      Boolean(case10Runtime) &&
      Boolean(case10Compare) &&
      !hasDate(case10Runtime!, "2024-10-25") &&
      !hasDate(case10Runtime!, "2024-10-30") &&
      case10Compare!.comparisonStatus !== "broad_failed" &&
      case10Compare!.comparisonStatus !== "broad_mismatch"
  };

  const case36Protection = {
    runtimeContains2025_08_06: case36Runtime ? hasDate(case36Runtime, "2025-08-06") : null,
    blockerReappeared:
      !case36Runtime ||
      !case36Compare ||
      hasDate(case36Runtime, "2025-08-06") ||
      case36Compare.comparisonStatus === "broad_failed" ||
      case36Compare.comparisonStatus === "broad_mismatch",
    bundleCount: case36Runtime?.normalizedEventCoverageSummary.eventBundleCount ?? null
  };

  const runtimeManifest = {
    phase: "Broader Rerun",
    attemptedCases,
    successfulRuntimeCases,
    failedRuntimeCases,
    runtimeGenerationMethodSummary:
      "Broader rerun execution using real local env with .env.local override, real DB, and real storage/OCR path via createCaseForUser + upsertPatientInput + uploadDocumentFile + createOcrJob; frozen earlier artifacts preserved; Case7 excluded from this rerun track.",
    ...
    rerunStoppedEarly: attemptedCases.length < BROAD_ORDER.length,
    comparisonStatuses: compareStatuses,
    perCaseLowEndRiskState,
    anyRegression,
    regressionCases,
    anyLowEndRiskPresent,
    watchlistCases: [WATCH_CASE],
  ...
  const rerunSummary = {
    phase: "Broader Rerun",
    attemptedCases,
    excludedCases: [`${EXCLUDED_CASE}: watch-track excluded from broader rerun scope`],
    caseStatuses: compareStatuses,
    statusCounts,
    lowEndRiskSummary: {
      cleared: perCaseLowEndRiskState.filter((item) => item.lowEndRiskState === "low_end_risk_cleared").map((item) => item.caseName),
      possible: perCaseLowEndRiskState.filter((item) => item.lowEndRiskState === "low_end_risk_possible").map((item) => item.caseName),
      present: perCaseLowEndRiskState.filter((item) => item.lowEndRiskState === "low_end_risk_present").map((item) => item.caseName)
    },
    regressionSummary: {
      anyRegression,
      regressionCases
    ...
    finalBroaderRerunResult:
      !anyRegression && !case3Watch.blockerEscalated && case10Protection.stableImproved && !case36Protection.blockerReappeared
        ? "broader_rerun_completed_without_new_blocker"
        : "broader_rerun_completed_with_followup_needed",
    generatedAt: new Date().toISOString()
  };

  await writeJson(MANIFEST_PATH, runtimeManifest);
  await writeJson(SUMMARY_PATH, rerunSummary);
}
