...
const RUNTIME_DIR = path.join(ROOT, "phase12_5_runtime");
const COMPARE_DIR = path.join(ROOT, "phase12_5_compare");
const MANIFEST_PATH = path.join(ROOT, "phase12_5_validation_manifest.json");
...
async function executeCaseRerun({
  ...
  try {
    const created = await createCaseForUser(investigator.id, {
      title: `Phase 12.5 ${pilotCase.caseName}`,
    ...
    return {
      ...
      track,
      runtimeSource: {
        method: "phase12_5_tailrisk_service_runner",
        generatedAt,
...
async function main() {
  ...
  let phase12SuccessfullyReducedTailRiskSet = false;
  ...
  for (const caseName of VALIDATION_ORDER) {
    ...
    phase12SuccessfullyReducedTailRiskSet ||= tailRiskState === "resolved" || tailRiskState === "reduced";
  ...
  const validationManifest = {
    ...
    timeoutLikeRuntimeFailureRecurred,
    phase12SuccessfullyReducedTailRiskSet,
    anyProductBlockingTailRiskCase,
