...
import InvestigatorNarrativePage from "./page.js";
...
describe("investigator narrative page", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("updates localized UI and API lang when the locale changes", async () => {
    ...
    const fetchMock = vi.fn((input: string) => {
      if (input === "/api/auth/me") {
        return Promise.resolve(
          Response.json({
            success: true,
            data: { user: { id: "user-1", role: "investigator" } }
          })
        );
      }

      if (input === "/api/cases/case-1/reports/investigator/narrative?lang=en") {
        return Promise.resolve(
          Response.json({
            success: true,
            data: {
              ...
              sections: [{ heading: "2024-03-07 | exam", paragraphs: ["On 2024-03-07, records were reviewed."], requiresReview: false }]
            },
      ...
      if (input === "/api/cases/case-1/reports/investigator/narrative?lang=ko") {
        return Promise.resolve(
          Response.json({
            success: true,
            data: {
              ...
              generatedAt: "2026-03-08T00:00:00.000Z",
              requiresReview: false,
              sections: [{ heading: "2024-03-07 | exam", paragraphs: ["2024-03-07에 기록을 검토했다."], requiresReview: false }]
            },
    ...
    const ui = await InvestigatorNarrativePage({
    ...
    await waitFor(() => {
      expect(screen.getByText("2024-03-07에 기록을 검토했다.")).toBeTruthy();
    });

    expect(fetchMock).toHaveBeenCalledWith(
      "/api/cases/case-1/reports/investigator/narrative?lang=ko",
